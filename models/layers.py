...
class ConstrainedConv2d(nn.Conv2d):
    ...
    def _initialize(self, input: torch.Tensor):
        ...
        with torch.no_grad():
                ...
                with torch.no_grad():
                    if self.padding_mode == 'zeros':
                        self.weight.data = self.weight.data[:, :,
                                           self.kernel_size[0] // 2 + 1:self.kernel_size[1] // 2 + 2,
                                           self.kernel_size[1] // 2 + 1: self.kernel_size[1] // 2 + 2]
            ...
            self.output_padding = self.inp_dims[-1] - (
                        (self.out_dims[-1] - 1) * self.stride[-1] - 2 * self.padding[-1] + 1 * (
                            self.weight.shape[-1] - 1) + 1)
