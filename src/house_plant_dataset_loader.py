...
class HousePlantCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/house_plant/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
        ...
        logger.info("HousePlantDataCreator init")
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/house_plant")
    images_list = list(p.glob("archive/house_plant_species/*/*.jpg"))
    images_list.extend(list(p.glob("archive/house_plant_species/*/*.jpeg")))
    images_list.extend(list(p.glob("archive/house_plant_species/*/*.png")))
    labels_ids = dict()
