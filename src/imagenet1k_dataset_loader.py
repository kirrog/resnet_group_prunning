...
class Imagenet1kCSTMDatasetCreator:
    ...
    Path("/media/kirrog/data/data/fqwb_data/data/imagenet1k/prepared"
        ...
        logger.info("ImagenetDataCreator init")
...
def converting_archive2format():
    new_size = (32, 32)
    train_part, test_part = 0.95, 0.05
    p = Path("/media/kirrog/data/data/fqwb_data/data/imagenet1k")
    output_path = p / "prepared"

    images_list = list((p / "archives/train").glob("*/*/*.jpg"))
    labels_ids = dict()
    ...
    valid_images_list = list((p / "archives/valid/archive_v").glob("*/*.jpg"))
    max_width = 0
    max_height = 0
    ...
    for img_path in tqdm(valid_images_list, desc="Validation read"):
        try:
            ...
            if resized_img_np.size == 32 * 32:
                ...
                print(resized_img_np.shape)
            valid_labels_id2img_list[label_id].append(resized_img_np)
        except:
            pass
...
if __name__ == "__main__":
    ...
    vehicles_dataset_creator = Imagenet1kCSTMDatasetCreator()
