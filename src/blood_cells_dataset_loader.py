...
class BloodCellsCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/blood_cells/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
        ...
        logger.info("BloodCellsDataCreator init")
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/blood_cells")
    images_list = list(p.glob("bloodcells_dataset/*/*.jpg"))
    labels_ids = dict()
    ...
    for i, img_path in enumerate(tqdm(images_list, desc="Reading")):
        label_name = str(img_path.name).split("_")[0]
...
if __name__ == "__main__":
    ...
    # blood_cells_dataset_creator = BloodCellsCSTMDatasetCreator()
    # test_dataloader = blood_cells_dataset_creator.create_loaders(create_test_dataloader=True)["test"]
    # train_valid_dataloaders = blood_cells_dataset_creator.create_loaders()
