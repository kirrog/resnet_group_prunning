...
class BreastHistCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/breasts_histopathology/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
        ...
        logger.info("BreastHistDataCreator init")
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/breasts_histopathology")
    images_list = list(p.glob("archive/*/*/*.png"))
    labels_ids = dict()
    ...
    for i, img_path in enumerate(tqdm(images_list, desc="Reading")):
        label_name = str(img_path.name).split("class")[-1][:-4]
...
if __name__ == "__main__":
    ...
    # breasts_histopathology_dataset_creator = BreastHistCSTMDatasetCreator()
    # test_dataloader = breasts_histopathology_dataset_creator.create_loaders(create_test_dataloader=True)["test"]
    # train_valid_dataloaders = breasts_histopathology_dataset_creator.create_loaders()
