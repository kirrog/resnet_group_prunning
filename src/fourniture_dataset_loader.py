...
class FournitureCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/fourniture/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
        ...
        logger.info("FournitureDataCreator init")
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/fourniture")
    images_list = list(p.glob("archive/image/*/*.png"))
    labels_ids = dict()
...
if __name__ == "__main__":
    ...
    # fourniture_dataset_creator = FournitureCSTMDatasetCreator()
    # test_dataloader = fourniture_dataset_creator.create_loaders(create_test_dataloader=True)["test"]
    # train_valid_dataloaders = fourniture_dataset_creator.create_loaders()
