...
class ChestXRayCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/chest_x_ray/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/chest_x_ray/archive/Chest_X_Ray_Dataset")
    images_list = list(p.glob("*/*.png"))
    images_list.extend(list(p.glob("*/*.jpg")))
    images_list.extend(list(p.glob("*/*.jpeg")))
    labels_ids = dict()
    train_labels_ids_stats = defaultdict(int)
    max_width = 0
    max_height = 0
    output_path = p.parent.parent / "prepared"
...
if __name__ == "__main__":
    ...
    # ChestXRay_dataset_creator = ChestXRayCSTMDatasetCreator()
    # test_dataloader = ChestXRay_dataset_creator.create_loaders(create_test_dataloader=True)["test"]
    # train_valid_dataloaders = ChestXRay_dataset_creator.create_loaders()
