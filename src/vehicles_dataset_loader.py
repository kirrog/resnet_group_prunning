...
class VehiclesCSTMDatasetCreator:
    def __init__(self, data_dir: Path = Path("/media/kirrog/data/data/fqwb_data/data/vehicles/prepared"),
                 batch_size: int = 1024,
                 random_seed: int = 42,
...
def converting_archive2format():
    ...
    p = Path("/media/kirrog/data/data/fqwb_data/data/vehicles")
    images_list = list(p.glob("archive/Vehicles/*/*.jpg"))
    images_list.extend(list(p.glob("archive/Vehicles/*/*.jpeg")))
    images_list.extend(list(p.glob("archive/Vehicles/*/*.png")))
    labels_ids = dict()
...
if __name__ == "__main__":
    ...
    # vehicles_dataset_creator = VehiclesCSTMDatasetCreator()
